...
@pytest.mark.parametrize("uv_version", ("0.10.11", "0.10.11+build.1"))
...
def test_install_sh_replaces_obsolete_uv(posix_harness: PosixHarness) -> None:
    ...
    assert result.returncode == 0, result.stderr
    assert "uv 0.5.9 does not satisfy stable >=0.10.11" in result.stdout
...
@pytest.mark.parametrize("version", ("0.10.11-alpha.1", "0.11.0-rc.1"))
def test_install_sh_replaces_prerelease_uv(
    ...
    assert result.returncode == 0, result.stderr
    assert f"uv {version} does not satisfy stable >=0.10.11" in result.stdout
    assert "uv-install" in posix_harness.calls()
...
def test_install_ps1_replaces_unrelated_pi_command(
    ...
    powershell_harness.add_uv("0.10.11")
